...
- 公開URL（独自ドメイン）: **https://isogecall.github.io/**
- 予備URL（GitHub Pages）: https://isogecall.github.io/
...
### 手順
1. ブラウザで **https://github.com/isogecall/isogecall.github.io** を開く
2. 直したいファイルをクリック（文章のほとんどは **`index.html`**）
3. 右上の **鉛筆マーク（✏️ Edit）** をクリック
4. 文字を書き換える（後述の「どこを直す？」を参照）
5. 右上の緑ボタン **「Commit changes」** を押す → そのまま保存
6. **1〜3分待つ** と https://isogecall.github.io/ に反映されます（反映されないときはブラウザを更新）
...
フォーム送信は **FormSubmit.co 経由で `admin@isogecall.com` にメール**が届きます（静的サイトなのでサーバー不要）。

- 届き先を変えるとき: `index.html` 内の `var MAIL_TO = "admin@isogecall.com";` のアドレスを書き換え
- **初回だけ有効化が必要**: 送信先アドレスを変えた場合、その新アドレス宛に FormSubmit から「Activate Form」の確認メールが届くので、一度だけリンクをクリックしてください（クリック後は自動でメールが届きます）
- 送信内容: 会社名・お名前・メール・電話・種別・本文 が表形式のメールで届きます。返信すると問い合わせ者へ返信できます（Reply-To設定済み）
